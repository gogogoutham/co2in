...
#if defined(NDEBUG)
# error "CO2IN cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6318ffd0526e1708e9a29d94131b26e719cdccff6b5e7cb96025cecf7f757748");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // CO2IN: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "CO2IN Signed Message:\n";
...
{
    // CO2IN: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // CO2IN: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // CO2IN: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // CO2IN: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xce19a31ad40668b464baa4d6362bbe87a83045446a7d19313934b1668908ec4a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040564e12227899c1a6375233d368d47f30a2d775d96164847015bae024a0deff4797b3ce6847c4917e90ec3bfb4f835f33e659c970f5e70a5cb8eef95059b4e2f") << OP_CHECKSIG;
        CBlock block;
        ...
        block.nNonce   = 1912283212;

        if (fTestNet)
        {
            block.nTime    = 1398659042;
            block.nNonce   = 3792573787;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x274be9e6ab49743e4a856171908216e2fc7d0f412f51d59a684b92ebd23a7dcb"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // CO2IN: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CO2INMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CO2INMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CO2INMiner(CWallet *pwallet)
{
    printf("CO2INMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("co2in-miner");
    ...
    try { loop {
        ...
        printf("Running CO2INMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CO2INMiner, pwallet));
}
